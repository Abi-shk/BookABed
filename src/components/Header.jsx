...
function Header({currentView, setCurrentView }) {
  ...
  return (
    <div> <header
    className={`fixed top-0 left-0 w-full flex justify-between items-center h-20 md:h-40 p-4 z-20 gap-3 transition-all duration-300 ${
  ...
  >
    ...
    <div className="flex items-center justify-around w-1/2 md:w-1/3">
      <i
        className={`bx bx-notepad text-2xl md:text-4xl cursor-pointer  ${
          scrolled ? 'text-indigo-500' : 'text-white'
        }`}
      ></i>
      <Link to='/home'> <h1 className="text-indigo-500 text-center text-xl md:text-4xl font-serif font-bold underline  cursor-pointer">
        Book<span className="text-red-500">A</span>Bed
      </h1></Link>
      {scrolled && (
        <div className="hidden md:flex items-center space-x-6">
          <button className={`text-lg md:text-2xl font-bold text-black p-2 hover:text-indigo-500 hover:border-b-4 border-indigo-500 h-[60px] md:h-[120px] ${ currentView === 'flights' ? 'text-indigo-500 border-b-4 border-green-60' : 'text-black bg-transparent' }`} onClick={() => setCurrentView('flights')}>
            Flights
          </button>
          <button className={`text-lg md:text-2xl font-bold text-black p-2 hover:text-indigo-500 hover:border-b-4 border-indigo-500 h-[60px] md:h-[120px] ${ currentView === 'hotels' ? 'text-indigo-500 border-b-4 border-green-60' : 'text-black bg-transparent' }`} onClick={() => setCurrentView('hotels')}>
            Hotels
          </button>
        </div>
      )}
    ...
    <div className="flex w-1/3 justify-start items-center space-x-5">
      ...
      <IoMdLogIn className='flex cursor-pointer w-8 h-8 text-indigo-500 md:hidden'/>
        <button
          className={`w-32 hidden md:flex justify-center items-center h-10 hover:bg-slate-300 hover:text-black rounded-full ${
            scrolled
