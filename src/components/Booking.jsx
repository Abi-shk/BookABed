...
function Booking() {
  ...
  if (fromValue.length > 1) {
    debouncedFetchLocations(fromValue, "from");
  } else {
    setFromLocations([]);
  }
}, [fromValue]);
...
useEffect(() => {
  if (toValue.length > 1) {
    debouncedFetchLocations(toValue, "to");
  } else {
    setToLocations([]);
  }
}, [toValue]);
...
const debouncedFetchLocations = debounce(fetchLocations, 500);
  ...
  onClick={() => {
    setFromValue(location.iataCode);
    setFromLocations([]); // Close suggestions after selection
  }}
>
                            {location.name}
                          </p>
  ...
  className='border-b p-2 cursor-pointer'
  onClick={() => {
    setToValue(location.iataCode);
    setToLocations([]); // Close suggestions after selection
  }}
>
                          {location.name}
                        </p>
